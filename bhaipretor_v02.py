...
for line in lines:
	...
	# These values can be used with "bol bhai" (cout) so they must be changed even if they are in cout statement
	...
	# it will change the values in other lines but not change in the output statement
	if "cout" not in line:
...
# executeble file with .exe format to the second cmd command
